...
\section{Parser, Validator, Metric Collector, etc.}
    ...
    \begin{tikzpicture}[
        ...
        \tikzstyle{class} = [draw, text width=3em, font={\small\ttfamily}, fill=white, anchor=north];
        \newcommand{\br}{\linebreak\rule{3em}{0.4pt}\linebreak}
        \node {formula\br}
            child {node [dashed] {unary\br formula}
                child {node {braces\br}}
            ...
            child {node [dashed] {binary\br formula\linebreak formula}
                child {node {and\br}} 
                child {node {or\br}} 
            }
            child {node {sequence\br formula[]\linebreak operand}} 
