...
proxy::Flow::Flow(const std::string& t, const Signature& s) : _text(t), _signature(s) { 
    /* that's it */ 
}

proxy::Flows& proxy::Flow::alt(char c, const solm::Formula& f) { 
    if (_alternatives.find(c) == _alternatives.end()) {
        solm::Variadic v;
        v += f;
        _alternatives[c] = std::pair<solm::Variadic, Flows>(v, Flows());
    }
    return _alternatives[c].second; 
}
